...
def export_attendance_data(
    ...
    with conn.cursor() as cur:
        ...
        for emp in employees:
            ...
            # Get ALL individual punches (not grouped) for smart pairing
            # Fetch one extra day before and after for overnight detection
            fetch_from = date_from - timedelta(days=1)
            fetch_to   = date_to   + timedelta(days=1)
            cur.execute("""
                SELECT punch_time, branch_id
                FROM attendance_logs
                WHERE employee_id = %s
                  AND punch_time::date BETWEEN %s AND %s
                ORDER BY punch_time
            """, (emp_id, fetch_from, fetch_to))
            all_punches_raw = cur.fetchall()
            ...
            def get_work_date(punch_time):
                if punch_time.hour < DAY_BREAK_HOUR:
            ...
            for p in all_punches_raw:
                ...
                work_day_punches[str(wd)].append({
                    "time": p["punch_time"],
                    "branch_id": p["branch_id"]
                })

            # Build punches dict with first/last per work day
            punches = {}
            for wd, ps in work_day_punches.items():
            ...
            for ds in sorted(all_dates):
                ...
                row = {
                    ...
                    "branch":        branch_names.get(punch["punch_branch_id"],
                ...
                # Process punch data — day break already applied so dates are correct
                if punch:
